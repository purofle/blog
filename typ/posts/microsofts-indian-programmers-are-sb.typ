...
...
```
这里的 `(0x7fd < uVar7 - 1)` 化简后为 `(uVar7 > 0x7fe)`，其中 `0x7fe` 为浮点数可保存的最大值。那么我们的 `_X` 也就是底数 `-1` 的二进制表示为 `0xBFF0000000000000`，经过计算后，`uVar7` 的值为 `0xbff`，因此 `(0x7fd < 0xbfe)` 为真，进入到了这个特殊运算分支。
